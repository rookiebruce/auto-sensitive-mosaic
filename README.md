...
| ![整帧像素化的处理前示意](docs/assets/before-safe-02.jpg) | ![针对高风险区域进行重码](docs/assets/after-heavy-02.jpg) |
| ![整帧像素化的处理前示意](docs/assets/before-safe-03.jpg) | ![针对高风险区域进行重码](docs/assets/after-heavy-03.jpg) |
...
- 针对横躺、仰身、手臂上举、肩髋线异常或髋点缺失的画面，启用异常姿态胸部兜底。
...
候选框通过 IoU 和归一化中心距离匹配。匹配成功后使用指数平滑更新位置；短时间检测不到时，轨迹保留 `8` 帧，减少闪烁和短暂漏检。
...
- 兜底框不会主动覆盖脸部，但可能比纯检测框多遮挡一些躯干、头发、手臂或背景区域，以降低漏帧概率。
- 横躺、仰身、手臂上举、肩髋线重叠或髋点缺失时，会启用更高召回的胸部异常姿态兜底。

### 4. 精准重码

候选框四周扩张约 `10%`，并设置最小安全边距，避免只遮住检测框中心。默认马赛克块大小为 `34 px`，遮挡强度更高，优先降低敏感部位漏出概率。

EraX 命中的精细部位框使用更局部的重码策略：边界扩张约 `4%`，马赛克块大小为 `28 px`。这样可以优先遮住乳头、阴部等小目标，同时尽量少影响脸部和整体画面。
...
- `abnormal_pose_breast_box()`：修复横躺、仰身、手臂上举等姿态下胸部兜底框偏低或失效的问题。
