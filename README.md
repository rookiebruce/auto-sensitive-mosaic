...
一个本地运行的视频隐私处理工具。它逐帧识别高风险人体敏感区域，对候选区域施加重度像素化；同时使用人体姿态作为“护栏”，把遮挡限制在合理的躯干/骨盆范围内，避免把脸、头发、肩部或大面积背景一起打码。
...
- 使用跨帧跟踪和平滑，降低马赛克闪烁。
- 使用 YOLO Pose 估计人体范围、肩线和髋线，但只作为位置约束，不主动生成大面积遮挡框。
...
YOLO Pose 提供人体框、肩部和髋部关键点。程序据此估算每个人的合理躯干/骨盆范围。姿态结果不会单独创建马赛克区域，只用于裁剪 NudeNet 检测框：
...
候选框四周只扩张约 `6%`，并设置最小安全边距，避免只遮住检测框中心。默认马赛克块大小为 `28 px`，遮挡强度高，但覆盖范围比早期“严格版”更小。
...
| 参数 | 默认值 | 作用 |
| --- | ---: | --- |
| 敏感检测阈值 | `0.18` | 越低越不容易漏检，但误遮挡增加 |
| 轨迹保留 | `5` 帧 | 减少短暂漏检造成的闪烁 |
| 姿态置信度 | `0.12` | 控制人体姿态护栏的敏感度 |
| 边界扩张 | `6%` | 防止只遮住检测框中心 |
| 马赛克块 | `28 px` | 数值越大，遮挡越强 |
